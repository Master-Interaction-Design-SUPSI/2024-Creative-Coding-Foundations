...
async function fetchAsteroidData() {
  const url = 'https://api.nasa.gov/neo/rest/v1/neo/browse?api_key=c3dbNnFufATvLjyGHXCigcRyPWu34aRFsdgrnmVW';
...
function getRandomAsteroidDiameter() {
  return fetchAsteroidData().then(asteroids => {
    if (asteroids.length > 0) {
      const randomAsteroid = asteroids[Math.floor(Math.random() * asteroids.length)];
      const diameterKm = randomAsteroid.estimated_diameter.kilometers.estimated_diameter_max;

      // log to see data
      console.log(`Diameter from NASA (km): ${diameterKm}`);

      // diameter
      const diameterInPixels = diameterKm * 4; // diameter * 4

      // log for api
      console.log(`Mapped diameter (pixels): ${diameterInPixels}`);

      const finalRadius = Math.max(8, Math.min(diameterInPixels / 2, 40)); //
      return finalRadius;  
    } else {
      return Math.random() * (40 - 8) + 8;  
    }
  });
}
...
function spawnEnemies() {
  if (spawnerId) {
    clearInterval(spawnerId); // no add more spawners
  }

  spawnerId = setInterval(() => {
    getRandomAsteroidDiameter().then(radius => {
      let x, y;
      ...
      const color = `hsl(${Math.random() * 360}, 50%, ${Math.random() * 20 + 45}%)`; // enemy color
      const angle = Math.atan2(canvas.height / 2 - y, canvas.width / 2 - x);

      const velocity = {
        x: Math.cos(angle) * 0.8, // same
        y: Math.sin(angle) * 0.8, // speed enemy
      };
