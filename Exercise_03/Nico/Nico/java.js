// Funzione per generare un numero intero casuale in un range
function getRandomInt(min, max) {
    return Math.floor(Math.random() * (max - min + 1)) + min;
}

// Funzione per generare ostacoli casuali, limitando a 2 ostacoli per riga e assicurandosi che non ci siano ostacoli nella riga più bassa
function generateRandomObstacles(numberOfObstacles) {
    const obstacles = [];
    const rowCount = Array(gridSize).fill(0); // Array che tiene traccia del numero di ostacoli per riga

    for (let i = 0; i < numberOfObstacles; i++) {
        let row, col;
        
        // Trova una riga in cui ci sono meno di 2 ostacoli, ma non nella riga più bassa
        do {
            row = getRandomInt(0, gridSize - 2);  // Esclude la riga più bassa (gridSize - 1)
        } while (rowCount[row] >= 2);  // Assicurati che ci siano meno di 2 ostacoli in quella riga

        // Genera una colonna casuale
        col = getRandomInt(0, gridSize - 1);

        // Direzione casuale (1 per destra, -1 per sinistra)
        const direction = getRandomInt(0, 1) === 0 ? 1 : -1;

        // Aggiungi l'ostacolo alla lista
        obstacles.push({ row, col, direction });

        // Incrementa il contatore degli ostacoli nella riga
        rowCount[row]++;
    }
...
// Funzione per cambiare la riga di un ostacolo in modo casuale
function changeObstacleRow(obstacle) {
    obstacle.row = getRandomInt(0, gridSize - 2); // Assegna una nuova riga casuale, ma non la riga più bassa
}
